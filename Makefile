...
PLUGIN_DIR := Placebryson.sketchplugin
PLUGIN_NAME := Placebryson
ZIP_NAME := Placebryson.zip
S3_BUCKET := s3://placebryson
...
publish:
	@make package
	@aws s3 cp $(ZIP_NAME) $(S3_BUCKET)/releases/placebryson-$(TRAVIS_TAG).zip
